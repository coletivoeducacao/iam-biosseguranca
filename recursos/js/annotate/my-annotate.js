...
(function () {
    ...
    set: function (k, v) {
      try { localStorage.setItem(k, v); } catch (e) {
        if (e && (e.name === "QuotaExceededError" || e.name === "NS_ERROR_DOM_QUOTA_REACHED"))
          setTimeout(function () { toast("Armazenamento cheio — exporte seus comentários antes de adicionar novos.", { kind: "error", duration: 8000 }); }, 0);
      }
    },
  };
  var COLORS = [
  ...
  var state = {
    ...
    panelOpen: false,
    activeId: null,
    filter: "open", // open | resolved | all | project
    query: "",
  ...
  function parseISO(iso) {
    ...
    // If it's an ISO timestamp without timezone offset, treat as UTC to prevent browser discrepancy
  ...
  function dbRead() {
    ...
    if (!Array.isArray(d.comments)) d.comments = [];
      ...
      d.comments.forEach(function (c) {
        if (c.createdAt) c.createdAt = toUTC(c.createdAt);
        if (c.updatedAt) c.updatedAt = toUTC(c.updatedAt);
        if (Array.isArray(c.replies)) {
          c.replies.forEach(function (r) {
            if (r.createdAt) r.createdAt = toUTC(r.createdAt);
            if (r.updatedAt) r.updatedAt = toUTC(r.updatedAt);
          });
        }
      });
    }
  ...
  function pageComments() {
    return projectComments().filter(function (c) { return c.page === PAGE; });
  }
  ...
  function patchComment(id, changes) {
    ...
    if (changes.editReply) {
      ...
      if (ri >= 0) { c.replies[ri] = Object.assign({}, c.replies[ri], { text: changes.editReply.text, updatedAt: new Date().toISOString() }); }
    }
  ...
  .an-empty kbd { font:600 11px var(--an-font); color: var(--an-muted); background: var(--an-surface-2);
  ...
  .an-page { display:block; margin:-2px 0 8px 30px; color:var(--an-muted);
    font:500 11px/1.35 var(--an-font); overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
  ...
  #__an_foot .an-footrow.an-four, #__an_foot .an-footrow.an-five { flex-wrap:wrap; }
  ...
  #__an_foot .an-footrow.an-four .an-fbtn, #__an_foot .an-footrow.an-five .an-fbtn { flex-basis:calc(50% - 4px); }
  #__an_foot .an-footsep { width:100%; height:0; border:0; border-top:1px solid var(--an-border); margin:2px 0; }
  ...
  function sizeOverlay() {
    if (!overlay || isSizing) return;
    isSizing = true;
    overlay.style.width = "0px";
    overlay.style.height = "0px";
    var docEl = document.documentElement;
    var body = document.body;
    var w = Math.max(docEl.clientWidth, docEl.scrollWidth, body ? body.scrollWidth : 0);
    var h = Math.max(docEl.clientHeight, docEl.scrollHeight, body ? body.scrollHeight : 0);
  ...
  function renderBlock(c) {
    ...
    var docW = Math.max(document.documentElement.clientWidth, document.documentElement.scrollWidth);
  ...
  function renderGeom(c) {
    ...
    if (viewportMismatch(g)) {
      ...
      badgeTitle.textContent = "Desenhado em " + g.vw + "px de largura — as posições podem variar nesta tela";
  ...
  function openComposer(x, y, draft) {
    ...
    pendingDraft = draft;
    var label = { highlight: "Destaque", shape: draft.geom && draft.geom.kind === "circle" ? "Círculo" : "Retângulo", pin: "Marcador", pen: "Desenho", block: "Seção" }[draft.type] || "Nota";
    composer.innerHTML = "";
    composer.appendChild(el("div", { class: "an-ctitle" }, [
      swatchDot(draft.color), document.createTextNode("Comentário (" + label + ")"),
    ...
    var ta = el("textarea", { class: "an-ta", rows: "3", placeholder: "Escreva seu comentário…" });
    composer.appendChild(ta);
    var save = el("button", { class: "an-primary", text: "Comentar" });
    var cancel = el("button", { class: "an-ghost", text: "Cancelar" });
    ...
    composer.appendChild(el("div", { class: "an-cfoot" }, [
      el("span", { class: "an-ckbd", text: (isMac ? "⌘" : "Ctrl") + "↵ para enviar" }),
      cancel, save,
    ]));
  ...
  function askName(onDone) {
    ...
    var input = el("input", { placeholder: "ex.: Maria Silva", value: state.author || "" });
    var btn = el("button", { text: "Começar avaliação" });
    var kids = [
      el("h3", { class: "an-nt", text: "Por favor, informe seu nome" }),
      el("p", { class: "an-nd", text: "Seu nome aparecerá em cada comentário para identificar suas anotações. Salvo neste dispositivo — você não precisará informar novamente." }),
    ];
    ...
    if (state.note) {
      kids.push(el("div", { class: "an-nnote" }, [
        el("span", { class: "an-nlbl", text: "Observação: " }),
    ...
    function done() {
      state.author = input.value.trim() || "Anônimo";
  ...
  function buildUI() {
    ...
    var tools = [
      ["cursor", "Navegar", "V"], ["highlight", "Destacar texto", "H"],
      ["rect", "Retângulo", "R"], ["circle", "Círculo", "C"],
      ["pen", "Desenho livre", "D"], ["pin", "Marcador", "P"],
    ];
    ...
    var colorBtn = el("button", { id: "__an_colorbtn", "data-tip": "Cor", title: "Cor", "aria-label": "Cor" });
    ...
    var listBtn = el("button", { class: "an-btn", "data-tip": "Comentários  ·  A", title: "Comentários  ·  A", "aria-label": "Comentários", html: ICONS.list });
    ...
    var offBtn = el("button", { class: "an-btn", "data-tip": "Ocultar ferramentas  ·  O", title: "Ocultar ferramentas  ·  O", "aria-label": "Ocultar ferramentas", html: ICONS.hide });
    offBtn.addEventListener("click", function () { setEnabled(false); });
    bar.appendChild(offBtn);
    root.appendChild(bar);

    launchEl = el("button", { id: "__an_launch", class: SIDE, html: ICONS.bubble + "<span>Anotações</span>" });
    ...
    helpEl = el("div", { id: "__an_help", class: SIDE }, [
      el("h3", { text: "Atalhos do teclado" }),
    ]);
    [["V", "Navegar"], ["H", "Destacar texto"], ["R", "Retângulo"], ["C", "Círculo"],
     ["D", "Desenho livre"], ["P", "Marcador"], ["A", "Painel de comentários"], ["O", "Mostrar / ocultar ferramentas"],
     ["Esc", "Cancelar"], ["?", "Ajuda de atalhos"]].forEach(function (row) {
      helpEl.appendChild(el("div", { class: "an-krow" }, [
        el("span", { text: row[1] }), el("kbd", { text: row[0] }),
      ]));
    });
    helpEl.appendChild(el("div", { class: "an-sep", style: "width:100%;margin:6px 0;" }));
    root.appendChild(helpEl);

    panel = el("div", { id: "__an_panel" });
    var header = el("div", { class: "an-ph" }, [
      el("h2", { text: "Comentários" }),
      el("span", { class: "an-pcount", id: "__an_sub", text: "0" }),
      el("button", { class: "an-hbtn", html: ICONS.upload, title: "Importar comentários de um arquivo JSON", "aria-label": "Importar comentários de um arquivo JSON", onclick: function (e) { e.stopPropagation(); pickImportFile(); } }),
      el("button", { class: "an-hbtn", html: ICONS.download, title: "Baixar comentários como JSON", "aria-label": "Baixar comentários como JSON", onclick: function (e) { e.stopPropagation(); exportComments(); } }),
      el("button", { class: "an-x", html: "&times;", "aria-label": "Fechar painel de comentários", onclick: closePanel }),
    ]);
    var search = el("div", { class: "an-search" }, [
      (function () { var s = el("span", { html: ICONS.search }); return s.firstChild; })(),
    ]);
    var searchInput = el("input", { placeholder: "Pesquisar comentários…" });
    ...
    [["open", "Abertos"], ["resolved", "Resolvidos"], ["all", "Nesta página"], ["project", "Projeto"]].forEach(function (f) {
  ...
  function setupBlockPlus() {
    plusBtn = el("div", { id: "__an_plus", title: "Comentar nesta seção",
  ...
  function setEnabled(on) {
    ...
    if (on) {
      ...
      if (launchEl) {
        ...
        launchEl.querySelector("span").textContent = n ? "Anotações (" + n + ")" : "Anotações";
  ...
  function setTool(t) {
    ...
    var hints = {
      highlight: "Selecione qualquer texto para destacar e comentar",
      rect: "Arraste para desenhar um retângulo",
      circle: "Arraste para desenhar um círculo",
      pen: "Desenhe livremente — solte para comentar",
      pin: "Clique em qualquer lugar para fixar um marcador"
    };
  ...
  function showHint(txt) {
    ...
    hintEl.appendChild(document.createTextNode(" para cancelar"));
  ...
  function updateCount() {
    ...
    if (sub) sub.textContent = String(state.filter === "project" ? projectComments().length : state.comments.length);
  }

  var TYPE_LABEL = { highlight: "Destaque", shape: "Forma", pin: "Marcador", pen: "Desenho", note: "Nota", block: "Seção" };
  function visibleComments() {
    var source = state.filter === "project" ? projectComments() : state.comments;
    return source.filter(function (c) {
      ...
      if (state.query) {
        var hay = ((c.text || "") + " " + (c.author || "") + " " +
          (state.filter === "project" ? pageLabel(c) : "") + " " +
  ...
  function renderPanel() {
    ...
    if (!list.length) {
      var msg = state.query
        ? "Nenhum comentário encontrado para “" + esc(state.query) + "”."
        : state.filter === "resolved"
          ? "Nenhum comentário resolvido ainda."
          : "Nenhum comentário ainda.<br>Selecione um texto ou escolha uma ferramenta na barra — tente <kbd>H</kbd> para destacar ou <kbd>P</kbd> para marcador.";
    ...
    list.forEach(function (c) {
      var idx = state.filter === "project" ? projectComments().indexOf(c) + 1 : state.comments.indexOf(c) + 1;
      ...
      var meta = el("div", { class: "an-cmeta" }, [
        avatarEl(c.author),
        el("span", { class: "an-author", text: c.author || "Anônimo" }),
        ...
        c.resolved ? el("span", { class: "an-rbadge", html: ICONS.check + "<span>Resolvido</span>" }) : null,
        el("span", { class: "an-when", text: fmtTime(c.createdAt), title: (parseISO(c.createdAt) ? parseISO(c.createdAt).toLocaleString("pt-BR") : "") }),
      ]);
      card.appendChild(meta);
      if (state.filter === "project")
        card.appendChild(el("div", { class: "an-page", text: pageLabel(c) }));
        ...
        c.replies.forEach(function (r) {
          var replyRow = el("div", { class: "an-reply" }, [
            ...
            el("span", { style: "flex:1;min-width:0" }, [
              ...
              el("span", { class: "an-rwhen", text: fmtTime(r.createdAt), title: (parseISO(r.createdAt) ? parseISO(r.createdAt).toLocaleString("pt-BR") : "") }),
            ]),
          ]);
          if (r.author === state.author) {
            var rAct = el("span", { style: "display:flex;gap:4px;flex:none;margin-left:6px" });
            var rDel = el("button", { class: "an-mini an-danger", html: ICONS.trash, title: "Excluir resposta" });
      ...
      var rin = el("textarea", { class: "an-ta", rows: "2", placeholder: "Responder… (Ctrl+↵ para enviar)" });
      rbox.appendChild(rin);
      var rsend = el("button", { class: "an-primary", style: "margin-top:6px;align-self:flex-end", text: "Responder" });
      rbox.appendChild(rsend);
      function submitReply() {
        if (!rin.value.trim()) return;
        var reply = { id: uid(), author: state.author || "Anônimo", text: rin.value.trim(), createdAt: new Date().toISOString() };
      ...
      var esave = el("button", { class: "an-primary", text: "Salvar" });
      ...
      var act = el("div", { class: "an-cact" }, [
        el("button", { class: "an-mini", html: ICONS.reply + "<span>Responder</span>", onclick: function (e) {
          e.stopPropagation(); rbox.classList.toggle("an-show"); rin.focus();
        } }),
        el("button", { class: "an-mini", html: (c.resolved ? "" : ICONS.check) + "<span>" + (c.resolved ? "Reabrir" : "Resolver") + "</span>", onclick: function (e) {
          e.stopPropagation();
          var updated = patchComment(c.id, { resolved: !c.resolved });
          if (updated) { mergeComment(updated); renderAll(); renderPanel(); }
        } }),
        c.author === state.author ? el("button", { class: "an-mini", html: ICONS.edit + "<span>Editar</span>", onclick: function (e) {
          e.stopPropagation();
          eta.value = c.text || "";
          ebox.classList.toggle("an-show");
          eta.focus();
        } }) : null,
        el("button", { class: "an-mini", html: ICONS.link, title: "Copiar link para este comentário", onclick: function (e) {
          e.stopPropagation(); copyLink(c.id);
        } }),
        el("button", { class: "an-mini an-danger", html: ICONS.trash, title: "Excluir", onclick: function (e) {
      ...
      card.addEventListener("click", function () {
        if (c.page !== PAGE) redirectToComment(c);
        else focusComment(c.id, false);
      });
  ...
  function buildExportAllPayload() {
    var all = dbRead().comments;
    if (!all.length) return null;
    // group by page so the consumer knows where each comment came from
    var pages = {};
    all.forEach(function (c) {
      var p = c.page || "(unknown)";
      if (!pages[p]) pages[p] = [];
      pages[p].push(c);
    });
    return {
      annotate: VERSION,
      kind: "annotate-export-all",
      exportedAt: new Date().toISOString(),
      project: CFG.project || "",
      currentPage: PAGE,
  ...
  function exportComments() {
    ...
    if (!payload) {
      toast("Nenhum comentário nesta página para exportar", { kind: "info" });
      return;
    }
    var comments = payload.comments;
    var slug = (PAGE || "page").replace(/[^a-z0-9]+/gi, "-").replace(/^-+|-+$/g, "").slice(0, 60) || "page";
    var stamp = localDateStamp();
    downloadJSON(payload, "anotacoes-" + slug + "-" + stamp + ".json");
    toast("Exportado " + comments.length + " comentário" + (comments.length === 1 ? "" : "s"), { kind: "success" });
  }

  function exportAllComments() {
    var payload = buildExportAllPayload();
    if (!payload) {
      toast("Nenhum comentário neste projeto para exportar", { kind: "info" });
      return;
    }
    var slug = (CFG.project || "project").replace(/[^a-z0-9]+/gi, "-").replace(/^-+|-+$/g, "").slice(0, 60) || "project";
    var stamp = localDateStamp();
    var pageCount = Object.keys(payload.pages).length;
    downloadJSON(payload, "anotacoes-todas-" + slug + "-" + stamp + ".json");
    toast("Exportado " + payload.totalComments + " comentário" + (payload.totalComments === 1 ? "" : "s") + " de " + pageCount + " página" + (pageCount === 1 ? "" : "s"), { kind: "success" });
  }

  function copyComments() {
    var payload = buildExportPayload();
    if (!payload) {
      toast("Nenhum comentário nesta página para copiar", { kind: "info" });
      return;
    }
    copyText(JSON.stringify(payload, null, 2), "Comentários copiados como JSON");
  }

  function copyAllComments() {
    var payload = buildExportAllPayload();
    if (!payload) {
      toast("Nenhum comentário neste projeto para copiar", { kind: "info" });
      return;
    }
    copyText(JSON.stringify(payload, null, 2), "Todos os comentários do projeto copiados como JSON");
  }
  ...
  function pickImportFile() {
    ...
    inp.addEventListener("change", function () {
      ...
      reader.onload = function () {
        ...
        catch (e) { toast("Esse arquivo não é um JSON válido", { kind: "error" }); return; }
        importComments(data);
      };
      reader.onerror = function () { toast("Não foi possível ler esse arquivo", { kind: "error" }); };
      reader.readAsText(f);
    });
    inp.click();
  }
  ...
  // Merge two reply arrays: deduplicate by ID (newest wins), sort by createdAt
  function mergeReplies(existingReplies, incomingReplies) {
    var map = {};
    (existingReplies || []).forEach(function (r) { if (r && r.id) map[r.id] = r; });
    (incomingReplies || []).forEach(function (r) {
      if (!r || !r.id) return;
      var prev = map[r.id];
      if (!prev) { map[r.id] = r; return; }
      // All timestamps are stored as pure UTC without offset, so direct comparison works without conversion
      var prevTs = prev.updatedAt || prev.createdAt || "";
      var incTs = r.updatedAt || r.createdAt || "";
      if (incTs > prevTs) map[r.id] = r;
    });
    var merged = [];
    for (var id in map) if (map.hasOwnProperty(id)) merged.push(map[id]);
  ...
  function importComments(data) {
    ...
    if (!incoming) { toast("Nenhum comentário encontrado nesse arquivo", { kind: "error" }); return; }
    var isExportAll = data.kind === "annotate-export-all";
    // Warn if a single-page export came from a different page
    if (!isExportAll && data.page && data.page !== PAGE)
      toast("Esses comentários são de uma página diferente — as posições podem não coincidir.", { kind: "info", duration: 6000 });

    // Build a lookup of all existing comments in the database (across all pages)
    var d = dbRead();
    var existingMap = {};
    d.comments.forEach(function (c, i) { if (c.id) existingMap[c.id] = i; });
    ...
    incoming.forEach(function (c) {
      ...
      // Always store createdAt and updatedAt as pure UTC with no timezone offset
      copy.createdAt = toUTC(copy.createdAt);
      copy.updatedAt = toUTC(copy.updatedAt || copy.createdAt);
      copy.replies.forEach(function (r) {
        if (r) {
          r.createdAt = toUTC(r.createdAt);
          if (r.updatedAt) r.updatedAt = toUTC(r.updatedAt);
        }
      });

      // Preserve original page for export-all; force current page for single-page exports
      if (!isExportAll) copy.page = PAGE;
      if (!copy.page) copy.page = PAGE;

      // Check if a comment with this ID already exists
      if (copy.id && existingMap.hasOwnProperty(copy.id)) {
        // Merge: keep properties from whichever has the most recent updatedAt (direct UTC comparison)
        var idx = existingMap[copy.id];
        var exist = d.comments[idx];
        var existTs = exist.updatedAt || exist.createdAt || "";
        var incTs = copy.updatedAt || copy.createdAt || "";
        var mergedReplies = mergeReplies(exist.replies, copy.replies);
        if (incTs > existTs) {
          // incoming is newer — use its properties, but merge replies
          copy.replies = mergedReplies;
          d.comments[idx] = copy;
        } else {
          // existing is newer — keep its properties, but merge replies
          exist.replies = mergedReplies;
        }
        merged++;
      } else {
        // Brand new comment
        if (!copy.id) copy.id = uid();
        d.comments.push(copy);
        existingMap[copy.id] = d.comments.length - 1;
        added++;
      }
    });

    if (!added && !merged) { toast("Nada novo para importar", { kind: "info" }); return; }
    dbWrite(d);
    load();
    var parts = [];
    if (added) parts.push("adicionado" + (added === 1 ? "" : "s") + " " + added);
    if (merged) parts.push("mesclado" + (merged === 1 ? "" : "s") + " " + merged);
    toast("Importação concluída: " + parts.join(", ") + " comentário" + ((added + merged) === 1 ? "" : "s"), { kind: "success" });
  }

  function copyLink(id) {
    var link = location.origin + location.pathname + location.search + "#an=" + id;
    function ok() { toast("Link copiado para a área de transferência", { kind: "success" }); }
    if (navigator.clipboard && navigator.clipboard.writeText)
      navigator.clipboard.writeText(link).then(ok, function () { prompt("Copiar link:", link); });
    else prompt("Copiar link:", link);
  }
  ...
  function deleteComment(c) {
    ...
    toast("Comentário excluído", {
      kind: "info", action: "Desfazer", duration: 5000,
      onAction: function () {
  ...
  function renderFooter() {
    ...
    footEl.appendChild(el("div", { class: "an-localnote" }, [
      ...
      el("span", { text: canShare
        ? "Salvo neste navegador. Baixe ou compartilhe para enviar seus comentários."
        : "Salvo neste navegador. Baixe para enviar seus comentários." }),
    ]));
    var allCount = dbRead().comments.length;
    footEl.appendChild(el("div", { class: "an-footrow" + (canShare ? " an-four" : "") }, [
      el("button", { class: "an-fbtn" + (n ? " an-pulse" : ""), title: "Baixar comentários desta página como JSON", html: ICONS.download + "<span>Baixar</span>", onclick: exportComments }),
      el("button", { class: "an-fbtn", title: "Copiar comentários desta página como JSON", "aria-label": "Copiar comentários como JSON", html: ICONS.copy + "<span>Copiar</span>", onclick: copyComments }),
      canShare ? el("button", { class: "an-fbtn", title: "Enviar comentários para " + state.share, html: ICONS.share + "<span>Compartilhar</span>", onclick: shareComments }) : null,
      el("button", { class: "an-fbtn", html: ICONS.upload + "<span>Importar</span>", onclick: pickImportFile }),
    ]));
    if (allCount > n) {
      footEl.appendChild(el("hr", { class: "an-footsep" }));
      footEl.appendChild(el("div", { class: "an-footrow" }, [
        el("button", { class: "an-fbtn", title: "Baixar TODOS os comentários de todas as páginas deste projeto", html: ICONS.download + "<span>Baixar tudo (" + allCount + ")</span>", onclick: exportAllComments }),
        el("button", { class: "an-fbtn", title: "Copiar TODOS os comentários de todas as páginas deste projeto", html: ICONS.copy + "<span>Copiar tudo</span>", onclick: copyAllComments }),
      ]));
    }
  }
  ...
  function copyText(text, okMsg) {
    function ok() { toast(okMsg || "Copiado para a área de transferência", { kind: "success" }); }
    function fail() { window.prompt("Copie isto:", text); }
    if (navigator.clipboard && navigator.clipboard.writeText)
  ...
  function shareSummary(comments) {
    ...
    var lines = visible.map(function (c, i) {
      var who = c.author || "Anônimo";
      var what = clipText(c.text, 280) || "(sem texto)";
      return (i + 1) + ". [" + (TYPE_LABEL[c.type] || c.type) + "] " + who + ": " + what;
    }).join("\n");
    if (comments.length > visible.length)
      lines += "\n… e mais " + (comments.length - visible.length) + " comentário" + (comments.length - visible.length === 1 ? "" : "s") + " no arquivo JSON.";
    return {
      subject: "Comentários da avaliação — " + (CFG.project || PAGE),
      body: "Avaliação de " + location.href + "\n\n" + lines +
        "\n\n(" + comments.length + " comentário" + (comments.length === 1 ? "" : "s") +
        ". O arquivo JSON completo mantém posições e respostas — anexe-o.)",
    };
  }
  ...
  function shareComments() {
    var comments = state.comments.slice();
    if (!comments.length) { toast("Nenhum comentário para compartilhar ainda", { kind: "info" }); return; }
    var dest = (state.share || "").trim();
    if (!dest) {
      toast("Nenhum destino de compartilhamento definido pelo autor — use Baixar", { kind: "info" });
      return;
    }
    ...
    var dlButton = btn("Baixar JSON", ICONS.download, false, function () { exportComments(); });

    var step2;
    if (isEmail) {
      step2 = step(2, el("span", {}, [
        document.createTextNode("Envie os comentários por e-mail para "),
        el("b", { text: dest }),
        document.createTextNode(". Abra seu aplicativo de e-mail abaixo (ou copie os dados) e "),
        el("b", { text: "anexe o arquivo do passo 1" }),
        document.createTextNode("."),
      ]), [
        btn("Abrir e-mail", ICONS.mail, false, function () {
        ...
        btn("Copiar e-mail", ICONS.copy, true, function () { copyText(dest, "Endereço de e-mail copiado"); }),
        btn("Copiar resumo", ICONS.copy, true, function () { copyText(sum.subject + "\n\n" + sum.body, "Resumo copiado"); }),
      ]);
    } else {
      var channelUrl = safeHttpUrl(dest);
      step2 = step(2, el("span", {}, [
        document.createTextNode("Publique os comentários no seu canal. Abra-o abaixo, cole o resumo copiado e "),
        el("b", { text: "anexe o arquivo do passo 1" }),
        document.createTextNode("."),
      ]), [
        channelUrl ? btn("Abrir canal", ICONS.share, false, function () { window.open(channelUrl, "_blank", "noopener"); }) : null,
        channelUrl ? null : btn("Copiar destino", ICONS.copy, true, function () { copyText(dest, "Destino copiado"); }),
        btn("Copiar resumo", ICONS.copy, true, function () { copyText(sum.subject + "\n\n" + sum.body, "Resumo copiado"); }),
      ]);
    }

    var box = el("div", { id: "__an_sharebox" }, [
      el("h3", { class: "an-st", text: "Compartilhe sua avaliação" }),
      el("p", { class: "an-sd", text: "Os comentários ficam salvos apenas neste navegador. Envie-os em dois passos — eles não são enviados automaticamente a nenhum lugar." }),
      el("div", { class: "an-sdest" }, [
        el("span", { html: isEmail ? ICONS.mail : ICONS.share }).firstChild,
        el("span", {}, [el("b", { text: isEmail ? "E-mail para: " : "Canal: " }), document.createTextNode(dest)]),
      ]),
      step(1, el("span", {}, [
        document.createTextNode("Baixe o arquivo de comentários "),
        el("b", { text: "(anotacoes-" + fileSlug + "-….json)" }),
        document.createTextNode("."),
      ]), [dlButton]),
      step2,
      el("button", { class: "an-sclose", text: "Concluído", onclick: closeShareDialog }),
    ]);
  ...
  function pageLabel(c) {
    try {
      if (c.url) {
        var u = new URL(c.url, location.href);
        return u.pathname + u.search;
      }
    } catch (e) {}
    var prefix = CFG.project ? CFG.project + ":" : "";
    return c.page && c.page.indexOf(prefix) === 0 ? c.page.slice(prefix.length) : (c.page || "Página desconhecida");
  }
  function redirectToComment(c) {
    var target = "";
    try {
      var u = new URL(c.url || pageLabel(c), location.href);
      if (u.origin === location.origin && /^https?:$/.test(u.protocol)) {
        u.hash = "an=" + encodeURIComponent(c.id);
        target = u.href;
      }
    } catch (e) {}
    if (!target) {
      toast("Não foi possível abrir a página deste comentário", { kind: "error" });
      return;
    }
  ...
  window.Annotate = {
    ...
    exportAll: function () { exportAllComments(); },
